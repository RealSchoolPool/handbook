...
## 7. Домашнее задание №5 — RU

```text
📌 ДЗ №5 — «Решает, но не печатает»

Срок: завтра к 11:00. Время: 30–40 минут.

Сегодня мы написали первый код бизнес-логики — core: функцию, которая РЕШАЕТ, что нужно пользователю, и ничего не печатает. Печатает отдельная часть. Это ДЗ — о том, зачем их разделили.

Правило: core ничего не печатает. cli ничего не решает.

1️⃣ core.py — довести функцию.
Получает список, возвращает список нужного. В core.py нет ни одного print и input, функция заканчивается на return.

2️⃣ cli.py — печатает то, что решил core:
  from core import filter_items
  for line in filter_items(items):
      print(line)
python cli.py должен напечатать ровно то же, что ваш project.py из ДЗ №4.

3️⃣ check_core.py — первая проверка без запуска программы:
  items = 3–4 строки из ваших данных
  expected = что функция должна вернуть — выписать ДО запуска
  print(filter_items(items) == expected)
Должно быть True.
Потом сломайте нарочно: поменяйте в core.py == на != → должно стать False. Верните обратно.
Проверка, которая никогда не говорит False, ничего не проверяет.

⭐ Со звёздочкой: возьмите из настоящих данных неудобную строку (дата словами, пустое поле) и решите с командой, что с ней должна делать программа.

4️⃣ Поездка к заказчику в другую школу перенесена на понедельник — и это плюс: покажем не рассказ, а работающую программу.
Команда [название] — файл customer-visit.md, полстраницы:
• какой вопрос решает программа (одна строка)
• что покажем: команда запуска и что она напечатает
• три вопроса заказчику — про ЕГО работу, не «вам нравится?»
• кто показывает — тот, кто выступал меньше всех
• кто записывает: имя и роль заказчика, одна цитата, одна цифра

⚠️ Никаких имён учеников в данных.
⚠️ Код объясняете построчно — неважно, откуда он.

Нет компьютера — функцию, expected и ответ «что напечатает» можно написать в тетради. Завтра запустим.

Сдать: в репозиторий команды, если получается; нет — в чат или принести завтра.

Полный текст и критерии: https://github.com/RealSchoolPool/handbook/blob/main/docs/HOMEWORK-05.md
